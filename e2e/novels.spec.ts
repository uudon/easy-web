...
test('Chinese visitors can open the independent novel shelf', async ({ page }) => {
  ...
  await expect(page.getByText('那七天里，11号舱一直是绿色的。')).toHaveCount(0)
  await expect(page.getByRole('link', { name: /03.*白点/ })).toBeVisible()
  await expect(page.getByRole('link', { name: /04.*十九米以下/ })).toBeVisible()
  await expect(page.getByRole('link', { name: /05.*正常/ })).toBeVisible()
  ...
  await expect
    ...
    .toBeGreaterThan(0)
  await page.getByRole('link', { name: /00.*简介/ }).click()
  await expect(page).toHaveURL(/\/zh-cn\/novels\/bei-wei-er-shi-qi-du\/description$/)
  await expect(page.getByRole('heading', { level: 1, name: '简介' })).toBeVisible()
  await expect(page.getByText('那七天里，11号舱一直是绿色的。')).toBeVisible()
  await page.getByRole('link', { name: /下一章.*鱼不吃了/ }).click()
  ...
  await expect(page.getByRole('link', { name: /上一章.*简介/ })).toBeVisible()
  ...
  await page.goto('/zh-cn/novels/bei-wei-er-shi-qi-du/chapter-05')
  await expect(page.getByRole('heading', { level: 1, name: '正常' })).toBeVisible()
  await expect(page.getByText('九月十四日十八点二十七分，韩启明在海图上画掉了向北的航线。')).toBeVisible()
})
